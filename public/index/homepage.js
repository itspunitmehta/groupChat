...
function showGroupMembers(){
    axios.get('http://54.160.208.96:4000/chat/join',{headers:{authanticate:token}})
    .then(response=>{
...
function sendmessage(e){
    ...
    axios.post('http://54.160.208.96:4000/chat/message',message,{headers:{authanticate:token}})
    .then(res=>{
        if(res.status===201){
...
function updateMessage(){
    ...
    axios.get(`http://54.160.208.96:4000/chat/recieve?id=${lastMsgId+1}`,{headers:{authanticate:token}})
    .then(res=>{
        if(res.status===200){
...
function createGroup(e){
    ...
    console.log(grpName);
    axios.post('http://54.160.208.96:4000/group/creategroup',grpName,{headers:{authanticate:token}})
    .then(response=>{
...
function addMemberToGroup(e){
    ...
    console.log(member);
    axios.post('http://54.160.208.96:4000/group/addmember',member,{headers:{authanticate:token}})
    .then(response=>{
        if(response.status ===201){
            document.getElementById('groupname-input').value = "";
            alert(response.data.message)
...
async function userGroups(){
    try {
        const allGrpDetails = await axios.get('http://54.160.208.96:4000/group/name',{headers:{authanticate:token}})
...
async function showGrpMsg(id){
    try {
        const grpMessages = await axios.get(`http://54.160.208.96:4000/group/getchat?id=${id}`,{headers:{authanticate:token}});
...
async function myfunc(e){
    ...
    const grpchat = await axios.post('http://54.160.208.96:4000/group/sendchat',message,{headers:{authanticate:token}})
    if(grpchat){
